...
void listaAluno()
{
    ...
    //TAMANHO DO VETOR e voltando no começo do arquivo com rewind
    fseek(arq, 0, SEEK_END);
    n = ftell(arq) / sizeof(aluno);
    aluno a[n];
    rewind(arq);

    //LENDO O ARQUIVO E FECHANDO O ARQUIVO
    ...
    for(i = 0; i <= n; i++)
    {
        if(a[i].cod_aluno == op)
        {
            ...
            printf("Estagio novo:\n <1> Iniciante\n <2> Intermediario\n"
                                    "<3> Avancado\n");
            scanf(" %d", &a[i].estagio);
            a[i].cod_aluno = a[i].cod_aluno;

            fseek(arq, -(i * sizeof(aluno)), SEEK_CUR);
